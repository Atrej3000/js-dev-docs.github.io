---
title: "TODO: Translate title"
description: "TODO: Translate description"
slug: "custom-errors"
track: "error-handling"
locale: "en"
sourceLocale: "uk"
translationStatus: "draft"
translationCompleteness: 0
---

# Translation draft

> TODO: Add the English lesson body for `error-handling/custom-errors`.
> The Ukrainian source lesson remains canonical and will continue to render as the public fallback until this file is marked as `published`.

## Source lesson

- Canonical source route: `/uk/docs/error-handling/custom-errors`
- Legacy source path: `uk/error-handling/custom-errors.html`

