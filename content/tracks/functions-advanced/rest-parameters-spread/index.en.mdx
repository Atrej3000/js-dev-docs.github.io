---
title: "TODO: Translate title"
description: "TODO: Translate description"
slug: "rest-parameters-spread"
track: "functions-advanced"
locale: "en"
sourceLocale: "uk"
translationStatus: "draft"
translationCompleteness: 0
---

# Translation draft

> TODO: Add the English lesson body for `functions-advanced/rest-parameters-spread`.
> The Ukrainian source lesson remains canonical and will continue to render as the public fallback until this file is marked as `published`.

## Source lesson

- Canonical source route: `/uk/docs/functions-advanced/rest-parameters-spread`
- Legacy source path: `uk/functions-advanced/rest-parameters-spread.html`

